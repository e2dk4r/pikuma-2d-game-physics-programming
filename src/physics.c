...
CollisionDetect(struct entity *a, struct entity *b, contact *contact)
{
  ...
  case VOLUME_TYPE_BOX | VOLUME_TYPE_BOX: {
    ...
    v2 support = Support(a, b, V2_UNIT_RIGHT);

    // simplex is array of points, max of 3
    v2 points[3] = {};
    points[0] = support;
    u32 pointCount = 1;
    ...
    while (1) {
      ...
      case 2: {
        ...
        if (IsSameDirection(ab, ao)) {
          ...
          //   ab × ao × ab
          direction = v2_cross(v2_cross(ab, ao), ab);
        } else {
          points[0] = pointA;
          pointCount = 1;
          direction = ao;
      ...
      case 3: {
        ...
        v2 abc = v2_cross(ab, ac);

        if (IsSameDirection(v2_cross(abc, ac), ao)) {
          if (IsSameDirection(ac, ao)) {
            points = {pointC, pointA};
            pointCount = 2;
            direction = v2_cross(v2_cross(ac, ao), ac);
          } else {
            // line
          }
        }
